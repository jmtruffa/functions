...
#' @export
#'
smonth <- function(df, date_column = "date", value_columns = -all_of(date_column)) {
  require(dplyr)
  require(lubridate)
...
#' @title Sumariza data por año
#' @description Esta functión recibe un dataframe y sumariza los datos por año, calculando la suma, media, mínimo y mnáximo.
#' @param df Un dataframe
#' @param date_column El nombre de la columna que contiene la fecha. "date" por default
#' @param value_columns Las columnas que se desean sumarizar. Todas las columnas por default
#' @return Un dataframe con la sumarización de los datos por año
#' @examples
#' \dontrun{syear(ccl, date_column = "date", value_columns = c("mepAL", "ccl3"))}
#' @import dplyr
#' @import lubridate
#' @export
#'
syear <- function(df, date_column = "date", value_columns = -all_of(date_column)) {
  require(dplyr)
  require(lubridate)
